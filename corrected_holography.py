...
def LP(x,m,p):
    #This function is for generating laguerre polynomials
...
#     """ This function takes in a modeling space like the one defined in the holography example
#     it then creates a modeling space native to lightpipes and generates a hermite gauss beam 
#     cross section using the library
#     """
#     um = .001 * mm
    
#     #create modeling space
#     GridDimension = X.shape[0]
#     GridSize = (X[-1,-1]- X[0,0]) * um
#     wavelength = 1 # this parameter seems to change nothing
#     Field = Begin(GridSize, wavelength, GridDimension)
    
#     hgbeam = GaussLaguerre(Field, w0, m, n, A = 1.0)
    
#     #return the field as an nd array to allow for computation within corrected holography
#     return hgbeam.field
...
#     hgbeam = GaussHermite(Field, w0, m, n, A = 1.0)
...
def correct_grating_arbitrary(beam, height, X, Z_curve, A_curve, x_curve, coefficients, num, pitch):
    ...
    run = numpy.zeros_like(arg, dtype=float)
    run += coefficients(0)
